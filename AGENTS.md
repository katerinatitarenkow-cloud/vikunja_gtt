...
## Local fork rules — token-efficient customization

This repository is being used as a personal Trello-like fork. Preserve upstream architecture and keep edits minimal.

- Do not scan or read the whole repository for routine tasks. Start with exact-symbol searches and the smallest relevant directory.
- Ignore `node_modules/`, `frontend/dist/`, `dist/`, `.git/`, `.github/`, `.vscode/`, `.zed/`, `build/`, `desktop/`, and `veans/` unless the requested task specifically concerns them.
- For UI-only changes, stay in `frontend/src/` unless a backend change is demonstrably required.
- For data model/business-logic changes, trace the minimal chain: model/migration -> API v2 -> frontend model/service -> UI.
- New HTTP functionality belongs in `/api/v2`; do not add new v1 routes.
- Prefer hiding or disabling unwanted upstream features over deleting large working subsystems, unless full removal is explicitly requested.
- Preserve SQLite as the default local database unless another database is explicitly requested.
- Before editing, state the small set of files you intend to inspect/change. Do not dump large unrelated files into context.
- Run only tests/lints relevant to changed areas first; expand only if necessary.
- Keep future upstream merging possible: avoid broad renames, formatting-only rewrites, and unrelated refactors.
