...
func Test_resolveDatabasePath(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "relative path with default rootpath stays inside Vikunja root",
			cfg: DatabasePathConfig{
				ConfiguredPath: "vikunja.db",
				RootPath:       "/opt/vikunja",
				ExecutablePath: "/opt/vikunja",
		...
		{
			name: "relative path ignores user data directory",
			cfg: DatabasePathConfig{
				ConfiguredPath: "vikunja.db",
				RootPath:       "/opt/vikunja",
				ExecutablePath: "/opt/vikunja",
...
func Test_resolveDatabasePath_Integration(t *testing.T) {
	...
	t.Run("with default rootpath keeps database inside Vikunja root", func(t *testing.T) {
		execPath, err := os.Executable()
		require.NoError(t, err)
		execDir := filepath.Dir(execPath)

		cfg := DatabasePathConfig{
			ConfiguredPath: "database/vikunja.db",
			RootPath:       execDir,
		...
		assert.Equal(t, filepath.Join(execDir, "database", "vikunja.db"), result)
	})
...
func TestMigrateLegacySQLiteDatabaseWithUserData(t *testing.T) {
	root := t.TempDir()
	legacyUserData := filepath.Join(t.TempDir(), "Vikunja")
	require.NoError(t, os.MkdirAll(filepath.Join(legacyUserData, "data"), 0o700))
	require.NoError(t, os.MkdirAll(filepath.Join(root, "database"), 0o700))

	legacyDB := filepath.Join(legacyUserData, "data", "vikunja.db")
	legacyWAL := legacyDB + "-wal"
	require.NoError(t, os.WriteFile(legacyDB, []byte("existing database"), 0o600))
	require.NoError(t, os.WriteFile(legacyWAL, []byte("pending wal"), 0o600))

	target := filepath.Join(root, "database", "vikunja.db")
	err := migrateLegacySQLiteDatabaseWithUserData(target, root, func() (string, error) {
		return legacyUserData, nil
	})
	require.NoError(t, err)

	got, err := os.ReadFile(target)
	require.NoError(t, err)
	assert.Equal(t, "existing database", string(got))
	wal, err := os.ReadFile(target + "-wal")
	require.NoError(t, err)
	assert.Equal(t, "pending wal", string(wal))

	// The old database remains untouched as a safety backup.
	_, err = os.Stat(legacyDB)
	require.NoError(t, err)
}

func TestMigrateLegacySQLiteDatabaseDoesNotOverwriteTarget(t *testing.T) {
	root := t.TempDir()
	legacyUserData := filepath.Join(t.TempDir(), "Vikunja")
	require.NoError(t, os.MkdirAll(filepath.Join(legacyUserData, "data"), 0o700))
	require.NoError(t, os.MkdirAll(filepath.Join(root, "database"), 0o700))
	...
	legacyDB := filepath.Join(legacyUserData, "data", "vikunja.db")
	target := filepath.Join(root, "database", "vikunja.db")
	require.NoError(t, os.WriteFile(legacyDB, []byte("legacy"), 0o600))
