...
//  3. Relative paths are always resolved inside service.rootpath. If rootpath is
//     unexpectedly empty, the executable directory is used as a safe fallback.
//
// The getUserDataDir parameter is kept for compatibility with the path tests and
// legacy-database migration helper. It is intentionally not used for new paths:
// portable desktop builds keep SQLite alongside Vikunja rather than in AppData.
func resolveDatabasePath(cfg DatabasePathConfig, _ func() (string, error)) (string, error) {
	if cfg.ConfiguredPath == "memory" {
		return "memory", nil
	}

	if filepath.IsAbs(cfg.ConfiguredPath) {
		return filepath.Abs(filepath.Clean(cfg.ConfiguredPath))
	}

	basePath := cfg.RootPath
	if basePath == "" {
		basePath = cfg.ExecutablePath
	}
	if basePath == "" {
		return "", fmt.Errorf("could not determine Vikunja root directory")
	}

	return filepath.Abs(filepath.Join(basePath, cfg.ConfiguredPath))
}

// migrateLegacySQLiteDatabase copies a database created by previous custom
// Windows builds from the old data locations to the new portable database
// directory. The legacy file is deliberately kept as a backup.
func migrateLegacySQLiteDatabase(targetPath, rootPath string) error {
	return migrateLegacySQLiteDatabaseWithUserData(targetPath, rootPath, getUserDataDir)
}

func migrateLegacySQLiteDatabaseWithUserData(targetPath, rootPath string, userDataDir func() (string, error)) error {
	...
	if _, err := os.Stat(targetPath); err == nil {
		...
		return fmt.Errorf("could not inspect target database %s: %w", targetPath, err)
	}

	candidates := []string{
		filepath.Join(rootPath, "data", "vikunja.db"),
		filepath.Join(rootPath, "vikunja.db"),
	}
	if legacyUserDataDir, err := userDataDir(); err == nil {
		candidates = append(candidates,
			filepath.Join(legacyUserDataDir, "data", "vikunja.db"),
			filepath.Join(legacyUserDataDir, "vikunja.db"),
		)
	} else {
		log.Warningf("Could not inspect legacy user-data database location: %v", err)
	}
	...
	for _, candidate := range candidates {
		...
		if err != nil {
			...
			return fmt.Errorf("could not inspect legacy database %s: %w", candidate, err)
		}
		if info.IsDir() {
			continue
		}
		if source == "" || info.ModTime().After(newest) {
			source = candidate
			newest = info.ModTime()
		}
	}
	...
	if err := copyDatabaseFile(source, targetPath); err != nil {
		return fmt.Errorf("could not migrate legacy database from %s to %s: %w", source, targetPath, err)
	}

	// Preserve SQLite sidecar files if they exist. WAL can contain committed
	// transactions that have not yet been checkpointed into the main database.
	for _, suffix := range []string{"-wal", "-journal"} {
		...
		if _, err := os.Stat(sourceSidecar); err != nil {
			...
			_ = os.Remove(targetPath + "-journal")
			return fmt.Errorf("could not inspect legacy SQLite sidecar %s: %w", sourceSidecar, err)
		}
		if err := copyDatabaseFile(sourceSidecar, targetPath+suffix); err != nil {
			_ = os.Remove(targetPath)
			_ = os.Remove(targetPath + "-wal")
			_ = os.Remove(targetPath + "-journal")
			return fmt.Errorf("could not migrate SQLite sidecar %s: %w", sourceSidecar, err)
		}
	}

	log.Infof("Copied legacy SQLite database from %s to portable location %s. The old database was kept as a backup.", source, targetPath)
	return nil
}
...
func initSqliteEngine() (engine *xorm.Engine, err error) {
	...
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil { // #nosec G703 -- path comes from Vikunja config/rootpath
		return nil, fmt.Errorf("could not create database directory %s: %w", filepath.Dir(path), err)
	}

	if err := migrateLegacySQLiteDatabase(path, rootPath); err != nil {
		return nil, err
	}
	...
	if isSystemDirectory(path) {
		log.Warningf("Database path (%s) appears to be in a system directory. This may cause issues. Please place Vikunja in a writable application directory or configure an absolute database path.", path)
	}
