...
func setUserAdmin(s *xorm.Session, identifier string, value bool) error {
	...
	u.IsAdmin = value
	if _, err = s.ID(u.ID).Cols("is_admin").Update(u); err != nil {
		return err
	}
	return models.SyncAdminAccessGroup(s, u)
}
...
var userSetAdminCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, args []string) {
		// This custom distribution uses is_admin for its own core administration
		// features (team/access management and Wialon settings), independently
		// from the optional upstream Pro Admin Panel. Upstream Pro routes remain
		// license-gated in the HTTP router.
		s := db.NewSession()
		defer s.Close()
