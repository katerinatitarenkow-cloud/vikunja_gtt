...
func TestHumaAdminAccessCreateUser(t *testing.T) {
	e, err := setupTestEnv()
	require.NoError(t, err)
	license.SetForTests([]license.Feature{license.FeatureAdminPanel})
	defer license.ResetForTests()
	...
	config.ServiceEnableRegistration.Set(false)
	defer config.ServiceEnableRegistration.Set(registrationEnabled)

	admin := promoteToAdmin(t, 1)
	const username = "access-created-user"
	const password = "averyl0ngpassword"
	body := `{"username":"` + username + `","password":"` + password + `"}`

	t.Run("admin creates an active non-admin who can log in", func(t *testing.T) {
		res := adminReq(t, e, http.MethodPost, "/api/v2/admin/access/users", admin, body)
		require.Equal(t, http.StatusCreated, res.Code, res.Body.String())
	...
	t.Run("regular user cannot create users", func(t *testing.T) {
		s := db.NewSession()
		regular, err := user.GetUserByID(s, 2)
		s.Close()
		require.NoError(t, err)

		res := adminReq(t, e, http.MethodPost, "/api/v2/admin/access/users", regular,
			`{"username":"access-forbidden","password":"averyl0ngpassword"}`)
	...
	t.Run("duplicate username is rejected", func(t *testing.T) {
		...
		assert.Contains(t, res.Body.String(), `"code":1001`)
	})
