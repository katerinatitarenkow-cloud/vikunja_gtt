...
func (tc *TaskComment) CreateWithTimestamps(s *xorm.Session, a web.Auth) (err error) {
	...
	if err := CreateSystemClientActivity(s, task.ProjectID, ClientActivityCommentCreated, tc.Author, "task", task.ID, tc.Comment, &ClientActivityMetadata{TaskTitle: task.Title}); err != nil {
		return err
	}
