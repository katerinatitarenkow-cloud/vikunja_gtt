...
func cleanupTaskMembersAfterTeamRemoval(s *xorm.Session, teamID int64, memberID int64) (err error) {
	...
	if len(taskIDs) > 0 {
		// Losing project access invalidates both direct and group-derived
		// assignee sources for these tasks. The semantic task↔group relation is
		// intentionally kept; reassigning the group after access is restored can
		// materialize the user again without granting permissions implicitly.
		_, err = s.In("task_id", taskIDs).
			And("user_id = ?", memberID).
			Delete(&TaskAssigneeSource{})
		if err != nil {
			return err
		}
