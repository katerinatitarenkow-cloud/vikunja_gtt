...
type Task struct {
	...
	// Operational work groups assigned to this task. They do not grant permissions;
	// their eligible members are materialized as normal task assignees.
	WorkGroups []WorkGroupSummary `xorm:"-" json:"work_groups" readOnly:"true" doc:"Operational work groups assigned to this task."`
...
func addMoreInfoToTasks(s *xorm.Session, taskMap map[int64]*Task, a web.Auth, view *ProjectView, expand []TaskCollectionExpandable) (err error) {
	...
	err = addWorkGroupsToTasks(s, taskIDs, taskMap)
	if err != nil {
		return
	}
...
func createTasks(s *xorm.Session, projectID int64, tasks []*Task, a web.Auth, updateAssignees bool, setBucket bool) (err error) {
	...
	for _, t := range tasks {
		...
		if err := CreateSystemClientActivity(s, t.ProjectID, ClientActivityTaskCreated, createdBy, "task", t.ID, "", &ClientActivityMetadata{TaskTitle: t.Title}); err != nil {
			return err
		}
...
func (t *Task) updateSingleTask(s *xorm.Session, a web.Auth, fields []string) (err error) {
	...
	// Capture the requested completion transition before updateDone potentially
	// resets repeating tasks back to open for their next occurrence.
	completedNow := !ot.Done && t.Done
	reopenedNow := ot.Done && !t.Done
	...
	*t = ot
	...
	if completedNow {
		if err := CreateSystemClientActivity(s, t.ProjectID, ClientActivityTaskCompleted, doer, "task", t.ID, "", &ClientActivityMetadata{TaskTitle: t.Title}); err != nil {
			return err
		}
	} else if reopenedNow {
		if err := CreateSystemClientActivity(s, t.ProjectID, ClientActivityTaskReopened, doer, "task", t.ID, "", &ClientActivityMetadata{TaskTitle: t.Title}); err != nil {
			return err
		}
	}
...
func hardDeleteTask(s *xorm.Session, t *Task) (err error) {
	...
	// Delete assignment-source metadata and work-group assignment relations
	// before the materialized assignee rows.
	if _, err = s.Where("task_id = ?", t.ID).Delete(&TaskAssigneeSource{}); err != nil {
		return err
	}
	if _, err = s.Where("task_id = ?", t.ID).Delete(&TaskWorkGroupAssignee{}); err != nil {
		return err
	}
