...
func DeleteUser(s *xorm.Session, u *user.User) (err error) {
	...
	relatedEntities := []struct {
		...
		{"user_id", &Favorite{}},
		{"owner_id", &APIToken{}},
		{"user_id", &AccessGroupMember{}},
		{"user_id", &WorkGroupMember{}},
		{"user_id", &TaskAssigneeSource{}},
		{"user_id", &UserProfile{}},
	}
	...
	// Keep operational groups valid when their leader account is removed.
	if _, err = s.Where("leader_user_id = ?", u.ID).Cols("leader_user_id").Update(&WorkGroup{LeaderUserID: 0}); err != nil {
		return err
	}
