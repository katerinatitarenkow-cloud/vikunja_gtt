...
func gateV2AdminRoutes() echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		licensedAdmin := feature(admin(next))
		instanceAdmin := admin(next)
		return func(c *echo.Context) error {
			path := c.Request().URL.Path
			// Team/access control and Wialon settings are custom core features of
			// this distribution. They require instance-admin privileges, but not
			// the optional upstream Pro Admin Panel license.
			if strings.HasPrefix(path, v2AdminPathPrefix+"/access") ||
				strings.HasPrefix(path, v2AdminPathPrefix+"/wialon") ||
				strings.HasPrefix(path, v2AdminPathPrefix+"/work-groups") {
				return instanceAdmin(c)
			}
			if strings.HasPrefix(path, v2AdminPathPrefix) {
				return licensedAdmin(c)
			}
			return next(c)
		}
	}
...
func registerAPIRoutesV2(e *echo.Echo, a *echo.Group, wsRateLimit echo.MiddlewareFunc) {
	...
	a.Use(gateGlobalFeaturePermissions())
