...
const (
	...
	WialonToken          Key = `wialon.token` // #nosec G101 -- config key name, not a credential
	WialonTimeoutSeconds Key = `wialon.timeoutseconds`
	WialonTrackMaxPoints Key = `wialon.trackmaxpoints`
...
func initDefaultConfig() {
	...
	ServiceEnableRegistration.setDefault(false)
	...
	DatabasePath.setDefault(ResolvePath("database/vikunja.db"))
	...
	WialonAPIURL.setDefault("https://hst-api.wialon.com")
	WialonToken.setDefault("")
	WialonTimeoutSeconds.setDefault(30)
	WialonTrackMaxPoints.setDefault(5000)
