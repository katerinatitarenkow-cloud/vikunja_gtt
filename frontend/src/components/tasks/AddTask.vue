...
.task-add,
.task-add .add-task__field { margin-block-end: 0; }
...
.task-add {
	...
	border-radius: 14px;
	box-shadow: 0 7px 24px rgba(31, 91, 73, .065);
}
...
.task-add .add-task__field {
	...
	.control { flex-shrink: 0; margin-block-end: 0; }
}
...
.task-input-wrapper {
	...
	.icon { color: var(--brand-text-muted); }
	.task-icon,
	:deep(.quick-add-magic-trigger-btn) { position: absolute; inset-block-start: .78rem; }
	:deep(.quick-add-magic-trigger-btn) { inset-inline-end: .75rem; }
	.task-icon { inset-inline-start: 1rem; }
}
...
.add-task-button {
	...
	@media screen and (max-width: $tablet) {
		...
		:deep(.icon) { margin: 0 !important; }
	}
}

.add-task-textarea { transition: border-color $transition, background-color $transition; resize: none; }
.textarea-empty { white-space: nowrap; text-overflow: ellipsis; }
.control .icon { transition: all $transition; z-index: 4; }
</style>
