...
<style lang="scss">
// GreenTech visual layer for Kanban. Kept separate from interaction/layout logic above.
.kanban {
	padding-block-end: .75rem;

	.ghost::after {
		border-color: color-mix(in srgb, var(--brand-forest) 42%, var(--brand-border));
		border-radius: 12px;
	}
	...
	.bucket {
		...
		border-radius: 14px;
		background: color-mix(in srgb, var(--brand-surface-soft) 78%, var(--white));
		box-shadow: 0 6px 18px rgba(31, 91, 73, .045);

		.task-item {
			background: transparent;
			padding: .35rem .55rem;

			&:first-of-type { padding-block-start: .55rem; }
			&:last-of-type { padding-block-end: .55rem; }
		}

		&.new-bucket {
			border: 1px dashed color-mix(in srgb, var(--brand-forest) 30%, var(--brand-border));
