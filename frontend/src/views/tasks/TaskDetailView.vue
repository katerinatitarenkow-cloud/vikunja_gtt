			...
			<div class="columns mbs-2">
				...
				>
						...
						>
							...
							<template v-else>
								<AssigneeList
									...
									class="mbs-2"
								/>
								<div v-if="task.workGroups?.length" class="mt-2">
									<span v-for="group in (task.workGroups ?? [])" :key="group.id" class="tag mr-1 mb-1">
										<Icon icon="users" />
					...
					<TaskChecklist
						:task-id="task.id"
						:can-write="canWrite"
						@state-changed="onChecklistStateChanged"
					/>
...
import TaskChecklist from '@/components/tasks/partials/TaskChecklist.vue'
...
function onChecklistStateChanged(state: ITaskChecklistState) {
	const doneAt = state.task_done_at ? new Date(state.task_done_at) : null
	task.value.done = state.task_done
	task.value.doneAt = doneAt

	const kanbanTask = kanbanStore.getTaskById(task.value.id)
	if (kanbanTask.task !== null) {
		kanbanStore.ensureTaskIsInCorrectBucket({
			...kanbanTask.task,
			done: state.task_done,
			doneAt,
		})
	}
}
...
function setActiveFields() {
	...
	activeFields.assignees = task.value.assignees.length > 0 || (task.value.workGroups?.length ?? 0) > 0
...
<style lang="scss" scoped>
// GreenTech task page polish
.task-view {
	background: var(--white);
	border: 1px solid var(--brand-border);
	border-radius: 16px;
	box-shadow: 0 10px 30px rgba(31, 91, 73, .06);
	padding: 1.25rem;
...
.subtitle {
	...
	a { color: var(--brand-forest); font-weight: 650; }
}

.details {
	padding: .8rem;
	margin-block-end: .75rem;
	background: color-mix(in srgb, var(--brand-surface-soft) 70%, var(--white));
	border: 1px solid var(--brand-border);
	border-radius: 12px;
...
.action-buttons {
	@media screen and (min-width: $tablet) {
		...
		border: 1px solid var(--brand-border);
		border-radius: 13px;
		box-shadow: 0 7px 22px rgba(31, 91, 73, .055);
	}
}
...
.scroll-to-comments-button {
	...
	box-shadow: 0 5px 16px rgba(31, 91, 73, .1);

	&:hover { background-color: var(--brand-surface-soft); color: var(--brand-forest-strong); }
}
</style>
